/**
 * Custom Prometheus Metrics for EU Funds Platform
 * Tracks: API latency, DB queries, external API health, business metrics
 */

// Metric types for Prometheus exposition format
...
// Database metrics
metrics.histogram('db_query_duration_seconds', 'Database query duration', [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1]);
...
metrics.gauge('external_api_health', 'External API health status (1=up, 0=down)');

// Business metrics
metrics.counter('user_registrations_total', 'Total user registrations');
metrics.counter('proposals_generated_total', 'Total proposals generated');
metrics.counter('proposals_submitted_total', 'Total proposals submitted');
metrics.counter('qes_signatures_total', 'Total QES signatures');
metrics.counter('qes_signature_errors_total', 'Total QES signature errors');
metrics.gauge('active_organizations_total', 'Total active organizations');

// Cache metrics
metrics.counter('cache_hits_total', 'Total cache hits');
metrics.counter('cache_misses_total', 'Total cache misses');
...
export function trackExternalAPI(api: string, success: boolean, durationMs: number): void {
  ...
  metrics.set('external_api_health', { api }, success ? 1 : 0);
}
