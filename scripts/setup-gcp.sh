...
echo "  REDIS_URL SENTRY_DSN QDRANT_API_KEY SMTP_PASS DB_PASS OPENAI_API_KEY ANTHROPIC_API_KEY"
...
ensure_secret_version "ANTHROPIC_API_KEY" "$ANTHROPIC_API_KEY"
...
print_warning "Production env vars are managed by the Cloud Build production pipeline (cloudbuild.production.yaml)"
...
echo "   NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY STRIPE_SECRET_KEY STRIPE_WEBHOOK_SECRET HEALTHCHECK_AUTH_TOKEN"
echo ""
echo "2. Create a Cloud Build trigger on master using cloudbuild.production.yaml"
echo "   Required substitutions: _GCS_BUCKET _GCS_KEY_FILENAME _QDRANT_URL _PRODUCTION_URL _CLOUDSQL_CONNECTION_NAME _DB_SOCKET_PATH _DB_NAME _DB_USER _VPC_CONNECTOR _VPC_EGRESS"
echo ""
echo "3. Deploy the application manually once, or let the trigger handle it:"
echo "   gcloud builds submit --config cloudbuild.production.yaml"
echo ""
echo "4. Optional legacy image-only build:"
echo "   gcloud builds submit --config cloudbuild.yaml"
echo ""
echo "5. Set up custom domain (replace your-domain.com):"
echo "   gcloud run domain-mappings create --service=fondeu-platform --domain=fondeu.your-domain.com --region=europe-west2"
echo ""
echo "6. Monitor costs in the GCP Console billing section"
echo ""
