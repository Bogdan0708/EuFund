...
**Deterministic preselect** (`lib/ai/agent/services/preselect.ts`, `POST /api/v1/projects/preselect`): server-side call selection + session bootstrap that replaces LLM-driven discovery for new projects. Three request modes (rank / confirm / override). Ranks top-5 calls by per-call vector similarity (pure max-score from `searchCalls`, which already dedupes). Three-branch decision policy: `selected` creates the session with `selectedCallId` + blueprint (when cached) + phase=`structuring` or `research`; `ambiguous` returns top-3 to the client without creating a session; `no_match` returns guidance without creating a session. Candidate list persists in `agent_sessions.planning_artifact` (versioned). Confirm mode validates the chosen callId via a `searchCalls` existence probe before session creation (400 `INVALID_CALL_ID` on miss). Override mode re-ranks on an existing session and mutates via `setSelectedCall` (409 `OUTLINE_FROZEN` / `CONCURRENCY_CONFLICT` on policy/CAS failures); it omits `blueprintKind`/`phase` from the response because `setSelectedCall` does not change them. Feature-flagged on `deterministic_preselect_enabled` with hard dependency on `managed_agent_writes_enabled`. Spec: `docs/superpowers/specs/2026-04-18-deterministic-preselect-design.md`.
