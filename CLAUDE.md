...
- **Stacked-PR CI gap.** `.github/workflows/ci.yml` filters `pull_request: branches: [main, master]`. PRs targeting feature branches get only secrets/GitGuardian/automerge — no build, no tests, no e2e. "CI green" on a stacked PR is meaningless until retargeted. Recipe to fire CI on a stacked PR after its dependent merges: `gh pr edit N --base master`, then `git rebase --onto origin/master <old-base-tip> <branch>`, then `git push --force-with-lease`. (See `~/.claude/projects/-home-godja/memory/aars/2026-04-30-pr2-stacked-stale-tool-surface-tests.md` for the case that surfaced this.)
- **Tool-surface unit tests must update with `WRITE_TOOL_NAMES` / `PHASE_4_BLOCKED_TOOL_NAMES` / `MANAGED_TOOLS` changes.** `tests/unit/managed/tools.test.ts` and `tests/unit/managed/executor.test.ts` assert literal counts (e.g. `MANAGED_TOOLS.length === 24`, `WRITE_TOOL_NAMES.size === 9`) and Phase-4 set membership — they often DON'T reference the changed tool by name, so the integration sweep won't catch the breakage. Always include `tests/unit/managed/` in the verification scope when changing the tool surface.
