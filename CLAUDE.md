...
│       ├── ai/             # AI endpoints (generate-proposal, match-grants, chat)
...
**Validation**: Zod for all request schemas, defined in `@/lib/validation/schemas.ts`. Key schemas: `extractedCallSchema`, `generateProposalSchema`, `wizardMatchCallsSchema`. Types are inferred from schemas.
