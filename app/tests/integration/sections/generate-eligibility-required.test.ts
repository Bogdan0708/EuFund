...
// Precondition failure: ELIGIBILITY_INPUT_REQUIRED
//
// Verifies that when ensureDraftingReady returns ok=false with
// code='ELIGIBILITY_INPUT_REQUIRED', the route returns 409 with the
// correct error envelope including bilingual messages and missing list.
...
describe('POST /sections/generate — eligibility input required', () => {
  beforeEach(() => vi.clearAllMocks())

  it('returns 409 ELIGIBILITY_INPUT_REQUIRED with missing list', async () => {
    ...
    expect(json.error.code).toBe('ELIGIBILITY_INPUT_REQUIRED')
    expect(json.error.messageRo).toBeTruthy()
    expect(json.error.messageEn).toBeTruthy()
    expect(json.error.missing).toEqual(['projectSummary'])
  })
})
