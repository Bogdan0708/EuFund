...
describe('POST /api/ai/predict-success oversight enforcement', () => {
  ...
  it('rejects oversight routing when the user belongs to multiple orgs and no orgId is provided', async () => {
    vi.resetModules();
    ...
    vi.doMock('@/lib/middleware/auth', () => ({
      withAIAuth: (_req: NextRequest, handler: (user: any) => Promise<any>) =>
        handler({ id: '123e4567-e89b-42d3-a456-426614174000', email: 'u@test.com', tier: 'free' }),
    }));
    vi.doMock('@/lib/legal/audit', () => ({ logAudit: vi.fn().mockResolvedValue(undefined) }));
    vi.doMock('@/lib/db', () => ({
      db: {
        query: {
          orgMembers: {
            ...
            findMany: vi.fn().mockResolvedValue([
              { orgId: '123e4567-e89b-42d3-a456-426614174222' },
              { orgId: '123e4567-e89b-42d3-a456-426614174333' },
            ]),
          },
        },
        insert: vi.fn(),
    ...
    expect(json.error.code).toBe('OVERSIGHT_ORG_REQUIRED');
  });
});
