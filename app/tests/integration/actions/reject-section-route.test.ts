// Integration test: POST /api/v1/agent-sessions/[id]/actions/reject-section
//
// Verifies:
//   - 400 on invalid body (missing sectionKey)
//   - 400 on invalid body (missing reason)
...
const rejectSectionSpy = vi.fn().mockResolvedValue({ newStateVersion: 5 })

vi.mock('@/lib/ai/agent/services/sections', () => ({
  rejectSection: rejectSectionSpy,
}))
...
describe('POST /api/v1/agent-sessions/[id]/actions/reject-section', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    isFeatureEnabledMock.mockResolvedValue(true)
    rejectSectionSpy.mockResolvedValue({ newStateVersion: 5 })
  })

  it('returns 400 when sectionKey is missing', async () => {
    const { POST } = await import(
      '@/app/api/v1/agent-sessions/[id]/actions/reject-section/route'
    )
    const res = await POST(
      new Request(
        'http://localhost/api/v1/agent-sessions/s1/actions/reject-section',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ reason: 'too vague', expectedStateVersion: 4 }),
        },
      ) as never,
  ...
  it('calls rejectSection service and returns 200 with UIStateSnapshot', async () => {
    const { POST } = await import(
      '@/app/api/v1/agent-sessions/[id]/actions/reject-section/route'
    )
    const res = await POST(
      new Request(
        'http://localhost/api/v1/agent-sessions/s1/actions/reject-section',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sectionKey: 'a', reason: 'too vague', expectedStateVersion: 4 }),
        },
      ) as never,
      { params: Promise.resolve({ id: 's1' }) } as never,
    )
    expect(res.status).toBe(200)
    expect(rejectSectionSpy).toHaveBeenCalledWith(
      expect.objectContaining({ userId: 'u1', sessionId: 's1' }),
      expect.objectContaining({ sessionId: 's1', sectionKey: 'a', reason: 'too vague', expectedStateVersion: 4 }),
    )
    const json = await res.json()
