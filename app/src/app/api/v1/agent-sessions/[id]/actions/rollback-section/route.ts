// POST /api/v1/agent-sessions/[id]/actions/rollback-section
//
// Rolls back a section to a previous version. Returns an updated
// UIStateSnapshot.
//
// Requires: session ownership, session must be active, outline must be
// frozen, target version must exist in section history.
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  const rl = await enforceRateLimit(req, {
    keyPrefix: 'action-rollback-section',
    keySuffix: user.id,
    maxRequests: 60,
    windowMs: 60 * 60 * 1000,
  ...
  const parsed = rollbackSectionBody.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
