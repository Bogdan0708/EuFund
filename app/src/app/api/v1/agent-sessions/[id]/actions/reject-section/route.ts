// POST /api/v1/agent-sessions/[id]/actions/reject-section
//
// Rejects a drafted section with a reason, sending it back for revision.
// Returns an updated UIStateSnapshot.
//
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  const rl = await enforceRateLimit(req, {
    keyPrefix: 'action-reject-section',
    keySuffix: user.id,
    maxRequests: 60,
    windowMs: 60 * 60 * 1000,
  ...
  const parsed = rejectSectionBody.safeParse(body)
  if (!parsed.success) {
    return NextResponse.json(
      {
        error: {
