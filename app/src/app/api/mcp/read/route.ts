// ── MCP Read Server — HTTP Route ──────────────────────────────────────────
// Validates the MCP SDK integration with Next.js App Router.
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const server = createReadServer(ctx)
  ...
  try {
    ...
    if (err instanceof ServiceError) {
      logger.warn({ requestId, err: err.message, code: err.code }, 'mcp:read service error')
    ...
    logger.error({ requestId, err }, 'mcp:read unexpected error')
