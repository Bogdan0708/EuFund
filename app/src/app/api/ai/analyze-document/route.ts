...
import { isLikelyNonTextPayload } from '@/lib/ai/sanitize';
...
export async function POST(request: NextRequest) {
  return withAIAuth(request, async (user) => {
    try {
      ...
      if ((projectContext && isLikelyNonTextPayload(projectContext)) || (callContext && isLikelyNonTextPayload(callContext))) {
        return NextResponse.json(
          Errors.validation(
            'context',
            'Câmpurile text trebuie să conțină doar text valid',
            'Context fields must contain valid text only'
          ).toResponse(),
          { status: 400 }
        );
      }
