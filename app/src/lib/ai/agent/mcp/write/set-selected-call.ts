...
export function registerSetSelectedCall(server: McpServer, ctx: ServiceContext): void {
  server.tool(
    'set_selected_call',
    "Set the current session's selected funding call. Requires the session to be active and the outline not yet frozen. Idempotent if the same callId is already selected. Always get explicit user confirmation or a structured UI action confirmation before calling — this is a write tool.",
    inputShape,
    async (args) => {
      try {
        requireSession(ctx)
