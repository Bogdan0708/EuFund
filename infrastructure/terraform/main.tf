# EU Funds Platform - AWS Infrastructure
# Region: eu-west-2 (London) - closest to Romania, GDPR compliant
# Alternative: europe-west3 (Frankfurt) on GCP
...
resource "aws_elasticache_replication_group" "main" {
  ...
  description                = "EU Funds Redis cluster"
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DATABASE_URL", value = "postgresql://${var.db_username}:${var.db_password}@${aws_rds_cluster.main.endpoint}:5432/eufunds" },
...
resource "aws_cloudfront_distribution" "main" {
  ...
  default_cache_behavior {
    ...
    forwarded_values {
      query_string = true
      headers      = ["Host", "Accept-Language", "Authorization"]
      cookies { forward = "all" }
    }
